            ...
            >
              <Dialog.Panel className="relative  w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="flex justify-end gap-2 pt-4">
                  <button
                    onClick={closeModal}
                    className="rounded-lg border border-black bg-black px-3 py-2 text-sm font-semibold text-white transition-all hover:bg-white hover:text-black"
                  >
                    <span>Cancel</span>
                  </button>
                  <button
                    disabled={watch("name") !== storeName}
                    className="group flex items-center justify-center gap-4 rounded-lg border-2 border-red-600 px-3 py-2 text-sm font-semibold text-red-600 disabled:opacity-40"
                    onClick={handleSubmit(onFormSubmit)}
                  >
                    <span>Yes, Delete</span>
                    {isDeleting && (
                      <div className="h-5 w-5 animate-spin rounded-full border-l-2 border-red-600" />
                    )}
                  </button>
                </div>
              </Dialog.Panel>
